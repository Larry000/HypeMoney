...
public class Updater extends BukkitRunnable {
    public void run() {
        Economy economy = HypingMoney.get().getEconomy();
        FileConfiguration config = HypingMoney.get().getConfig();
        for (Player player : Bukkit.getOnlinePlayers()) {
            ...
            for (ItemStack content : player.getInventory().getContents()) {
                if (content != null && content.getType() != Material.AIR && content.getItemMeta() != null) {
                    String coin = (String)HypingMoney.get().getMaterialHashMap().get(content.getType());
                    if (coin != null) {
            ...
            if (HypingMoney.get().getIgnoredPlayers().contains(player)) {
                oldAmount = currentAmount;
            } else if (HypingMoney.get().getMoneyHashMap().get(player) != null) {
                oldAmount = ((Integer)HypingMoney.get().getMoneyHashMap().getOrDefault(player, Integer.valueOf(0))).intValue();
            }
            if (oldAmount != currentAmount && (
                    currentAmount > oldAmount || oldAmount != 0)) {
                HashMap<String, Material> hashMap = new HashMap<>();
                HypingMoney.get().getMaterialHashMap().keySet().forEach(material -> hashMap.put((String)HypingMoney.get().getMaterialHashMap().get(material), material));
            ...
            HypingMoney.get().getMoneyHashMap().put(player, Integer.valueOf(money));
        }
    }
    ...
    public static void updateCoins(Player player, int oldAmount, int currentAmount, HashMap<String, Material> coinTypes) {
        ...
        if (difference > 0) {
            if (Math.abs(difference) >= goldWorth) {
                ...
                Bukkit.getScheduler().runTask((Plugin)HypingMoney.get(), () -> addItemToInventory(player, MMOItems.plugin.getMMOItem(MMOItems.plugin.getTypes().get("MATERIAL"), config.getString("coins.gold.id")).newBuilder().build(), coinsToAdd));
                difference -= coinsToAdd * goldWorth;
            }
            if (Math.abs(difference) >= silverWorth) {
                int coinsToAdd = Math.abs(difference) / silverWorth;
                Bukkit.getScheduler().runTask((Plugin)HypingMoney.get(), () -> addItemToInventory(player, MMOItems.plugin.getMMOItem(MMOItems.plugin.getTypes().get("MATERIAL"), config.getString("coins.silver.id")).newBuilder().build(), coinsToAdd));
                difference -= coinsToAdd * silverWorth;
            }
            if (difference != 0) {
                int coinsToAdd = Math.abs(difference) / bronzeWorth + ((Math.abs(difference) % bronzeWorth == 0) ? 0 : 1);
                Bukkit.getScheduler().runTask((Plugin)HypingMoney.get(), () -> addItemToInventory(player, MMOItems.plugin.getMMOItem(MMOItems.plugin.getTypes().get("MATERIAL"), config.getString("coins.bronze.id")).newBuilder().build(), coinsToAdd));
            }
            return;
        }
    ...
    public static int getCoinValue(Material material) {
        return HypingMoney.get().getConfig().getInt("coins." + (String)HypingMoney.get().getMaterialHashMap().get(material) + ".worth");
    }
        ...
        for (int i = 0; i < fullStacks; i++) {
            ...
            if (!leftoverItems.isEmpty())
                for (ItemStack leftover : leftoverItems.values())
                    Bukkit.getScheduler().runTask((Plugin)HypingMoney.get(), () -> player.getWorld().dropItem(player.getLocation(), leftover));
        }
